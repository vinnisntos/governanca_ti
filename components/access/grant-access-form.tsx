...
export function GrantAccessForm({
  ...
  return (
    <form action={action} className="space-y-4">
      <Field label="Colaborador" htmlFor="profile_id" required>
      ...
      {systemId === OTHER_SYSTEM_VALUE ? (
        <Field label="Nome do sistema" htmlFor="requested_system_name" required>
          <Input
      ...
      <Field
        label="Motivo da concessão"
        htmlFor="justification"
        required
        hint={`Por que este acesso está sendo concedido (mínimo 10 caracteres). ${justification.length}/${JUSTIFICATION_MAX}`}
      >
        <Textarea
          id="justification"
          name="justification"
      ...
      <div className="flex justify-end">
        <SubmitButton variant="primary" pendingLabel="Concedendo...">
          Conceder acesso
        </SubmitButton>
      </div>
    </form>
